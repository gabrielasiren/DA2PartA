...
```{r A.1.2 Age histogram, fig.dim = c(4,4)}
...
```{r A.1.2 Income histogram, fig.dim = c(4,4)}
...
```{r A.1.2 Kids histogram, fig.dim = c(4,4)}
...
```{r A.1.2 Aggregate Boxplot, fig.dim = c(4,4)}
...
```{r A.1.2 Segmented Boxplot, fig.dim = c(4,4)}
...
# data for suburbMix
mz <- mean(suburb_mixdata$income)
sz <- sd(suburb_mixdata$income)

# data for MovingUp
mu <- mean(movingUpData$income)
su <- sd(movingUpData$income)
...
tM <- cbind(meM,lclM,uclM)

ciData <- rbind(t,tT,tU,tS,tM)
colnames(ciData) <- c("ME","LCL","UCL")
rownames(ciData) <- c("Aggregate","Travellers","Urban Hip","Suburb Mix","Moving Up")

kable(ciData,
      caption="90% C.I for Income, both Aggregate and Segmented",
      format="simple",
      digits=2)
...
finaltTest <- map_df(list(tTestAgg,tTestTra,tTestUrb,tTestSub,tTestMov), tidy)

finaltTest[,c("method","alternative")] <- NULL


finaltTest <- cbind(c("Aggregate","Travellers","Urban Hip","Suburb Mix","Moving Up"),
finaltTest)
colnames(finaltTest) <- c("Segments","Estimate","Statistic","P.Value", "Parameter","Conf.Low","Conf.High")
#rownames(finaltTest) <- c("Aggregate","Travellers","Urban Hip","Suburb Mix","Moving Up")

kable(finaltTest,
      caption = "T-test table for Income, both Aggregate and Segmented",format="simple", digits = 3)
